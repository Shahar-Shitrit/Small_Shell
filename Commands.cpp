...
{
  SmallShell &smash = SmallShell::getInstance();
  c_jobs->removeFinishedJobs(); // added
  int signal = 0;
  char* minus_signal = nullptr;
  if(c_args[1] != nullptr) //added
  {
    minus_signal = c_args[1]; //signal num is 1-31
...
JobsList::~JobsList()
{
  for (int i = jobs_list.size()-1; i>=0; i--)
  {
    ...
    delete temp; //to check 
  }
}

void JobsList::addJob(Command *cmd, bool isStopped) //not done
{
  removeFinishedJobs();
